...
Covariance::Covariance(
    ...
    : U(U_), tt(tt_), V(V_), V_prime(V_prime_), sig(sig_), Ju_f(Ju_f_), Jp_f(Jp_f_), Jp_Ju_f(Jp_Ju_f_),
      Jp_Jp_Ju_f(Jp_Jp_Ju_f_) {
    mp1 = U.shape()[0];
    D = U.shape()[1];
    K = V.shape()[0];
    J = Jp_f.dx[0].size();
...
xt::xtensor<double, 2> Covariance::L(
    const std::vector<double> &p, const xt::xtensor<double, 1> &sig_
) const {
    const auto &sigma = sig_.size() == 0 ? sig : sig_;
        ...
        for (std::size_t d1 = 0; d1 < D; ++d1)
            for (std::size_t m = 0; m < mp1; ++m)
                for (std::size_t d2 = 0; d2 < D; ++d2)
                    L1_(k, d1, m, d2) = J_F(m, d1, d2) * V(k, m) * sigma(d2);

    auto L1 = xt::reshape_view(L1_, {K * D, mp1 * D});

    xt::xtensor<double, 2> L0_sig;
    bool has_custom_sig = (sig_.size() > 0);
    ...
    if (has_custom_sig) {
        ...
        for (std::size_t k = 0; k < K; ++k) {
            for (std::size_t d = 0; d < D; ++d) {
                for (std::size_t m = 0; m < mp1; ++m) {
                    L0_(k, d, m, d) = V_prime(k, m) * sig_(d);
                }
            }
        }

        L0_sig = xt::reshape_view(L0_, {K * D, mp1 * D});
    }

    const auto L = xt::eval(L1 + (has_custom_sig ? L0_sig : L0));
...
xt::xtensor<double, 3> Covariance::Jp_L(const std::vector<double> &p, const xt::xtensor<double, 1> &sig_) const {
    const auto &sigma = sig_.size() == 0 ? sig : sig_;
            ...
            for (std::size_t m = 0; m < mp1; ++m)
                for (std::size_t d2 = 0; d2 < D; ++d2)
                    for (std::size_t j = 0; j < J; ++j)
                        J_(k, d1, m, d2, j) = H_F(m, d1, d2, j) * V(k, m) * sigma(d2);
...
xt::xtensor<double, 4> Covariance::Hp_L(const std::vector<double> &p, const xt::xtensor<double, 1> &sig_) const {
    const auto &sigma = sig_.size() == 0 ? sig : sig_;
                ...
                for (std::size_t d2 = 0; d2 < D; ++d2)
                    for (std::size_t j1 = 0; j1 < J; ++j1)
                        for (std::size_t j2 = 0; j2 < J; ++j2)
                            H_(k, d1, m, d2, j1, j2) = T_F(m, d1, d2, j1, j2) * V(k, m) * sigma(d2);
...
xt::xtensor<double, 2> Covariance::operator()(const std::vector<double> &p, const xt::xtensor<double, 1> &sig_) const {
    const xt::xtensor<double, 1> sigma = sig_.size() == 0 ? sig : sig_;
    auto Lp = L(p, sigma);
...
xt::xtensor<double, 3> Covariance::Jacobian(const std::vector<double> &p,const xt::xtensor<double, 1> &sig_) const {
    const auto Lp = L(p, sig_);
    const auto Jp_Lp = Jp_L(p, sig_);
