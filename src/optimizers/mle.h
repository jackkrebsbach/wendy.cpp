...
struct J_wnll {
    ...
    ): U(U_), tt(tt_), V_prime(V_prime_), L(L_), g(g_), b(b_), JU_g(JU_g_), Jp_f(Jp_f_),
    ...
    xt::xtensor<double, 2> operator()(const std::vector<double> &p) const {
        ...
        for (int i = 0; i < p.size(); ++i) {
            // Shared values in parts
            const auto partial_p_Fp = xt::view(Jp_F3p, xt::all(), xt::all(), i);
            const auto partial_p_gp = xt::ravel<xt::layout_type::column_major>(
                xt::linalg::dot(xt::eval(partial_p_Fp), V_prime));
            //Parts
            //1
