...
inline J_f_functor build_J_f(const std::vector<std::vector<SymEngine::Expression> > &J_f_symbolic, const size_t &D,
                             const size_t &J) {
    const auto dx = build_jacobian_visitors(J_f_symbolic, D, J); // Symengine object to call numerical input
    return J_f_functor(dx);
}

inline H_f_functor build_H_f(const std::vector<std::vector<std::vector<SymEngine::Expression> > >&H_f_symbolic, const size_t &D,
                             const size_t &J) {
